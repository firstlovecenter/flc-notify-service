...
const sendPush = async (request, response) => {
    ...
    try {
        ...
        const failures = res.responses.reduce((acc, r, i) => {
            if (!r.success) {
                acc.push({
                    index: i,
                    token: tokens[i],
        ...
        console.log('[PUSH] ✓ FCM multicast complete. success:', res.successCount, 'failure:', res.failureCount);
        ...
        console.error('[PUSH] ❌ Push send error:', error instanceof Error ? error.message : 'Unknown error');
